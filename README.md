# ide.ankb — C++ Online IDE

![logo](https://i.ibb.co/Gfg90tSr/ide-ankb.jpg)

A fast, dark, VS Code–style online IDE for C++ (branded as **ide.ankb**), with a Node.js execution
...
Best for public demo, no backend hosting needed. Uses included `functions/api/[[path]].js` which automatically tries Judge0 CE → Piston → Wandbox.
...
6. Environment variables (optional):
   ...
   - Leave empty to auto-try: Judge0 CE → Piston public API (whitelist 401) → Wandbox
...
### 2. Piston / Wandbox / Judge0 CE Public API — No Backend (free, no self-host)

If `BACKEND_URL` is NOT set, the Pages Function automatically tries:
1. **Judge0 CE** (`JUDGE0_API_URL`) — **recommended after Piston 401**, self-host or RapidAPI
   - Self-host: `docker run -d -p 2358:2358 judge0/judge0:1.13.1` + set `JUDGE0_API_URL=http://your-ip:2358`
   - RapidAPI: https://rapidapi.com/judge0-official/api/judge0-ce (set `JUDGE0_API_KEY` + `JUDGE0_API_HOST`)
   - Language ID 54 = C++17 GCC
2. **Piston** (`https://emkc.org`) — but since **2026-02-15 it is whitelist-only (401)**, see https://github.com/engineer-man/piston#public-api
3. **Wandbox** (`https://wandbox.org`) fallback — still free and public, used automatically when Piston 401s, but may return `Resource temporarily unavailable` under load → fallback to Judge0

Original Piston example:
...
> **⚠️ Piston 401 fix (2026-02-15):** Public Piston API now returns `{"message":"Public Piston API is now whitelist only"}` with 401. Our Functions (`functions/api/[[path]].js` and `worker.js`) now automatically fallback to Wandbox (`https://wandbox.org/api/compile.json`) which is still public. If Wandbox also fails, deploy your own backend (option 3) and set `BACKEND_URL`.
